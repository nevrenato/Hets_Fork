...
instance GetRange Annote_word where
  ...
  rangeSpan x = case x of
    Annote_word a -> joinRanges [rangeSpan a]
    Comment_start -> []

instance GetRange Annote_text where
  getRange = const nullRange
  rangeSpan x = case x of
    Line_anno a -> joinRanges [rangeSpan a]
    Group_anno a -> joinRanges [rangeSpan a]
...
instance GetRange Annotation where
  getRange x = case x of
    Unparsed_anno _ _ p -> p
    Display_anno _ _ p -> p
    List_anno _ _ _ p -> p
    Number_anno _ p -> p
    Float_anno _ _ p -> p
    String_anno _ _ p -> p
    ...
    Semantic_anno _ p -> p
  rangeSpan x = case x of
    Unparsed_anno a b c -> joinRanges [rangeSpan a, rangeSpan b,
                                       rangeSpan c]
    Display_anno a b c -> joinRanges [rangeSpan a, rangeSpan b,
                                      rangeSpan c]
    List_anno a b c d -> joinRanges [rangeSpan a, rangeSpan b,
                                     rangeSpan c, rangeSpan d]
    Number_anno a b -> joinRanges [rangeSpan a, rangeSpan b]
    Float_anno a b c -> joinRanges [rangeSpan a, rangeSpan b,
                                    rangeSpan c]
    String_anno a b c -> joinRanges [rangeSpan a, rangeSpan b,
                                     rangeSpan c]
    Prec_anno a b c d -> joinRanges [rangeSpan a, rangeSpan b,
                                     rangeSpan c, rangeSpan d]
    Assoc_anno a b c -> joinRanges [rangeSpan a, rangeSpan b,
                                    rangeSpan c]
    Label a b -> joinRanges [rangeSpan a, rangeSpan b]
    Prefix_anno a b -> joinRanges [rangeSpan a, rangeSpan b]
    Semantic_anno a b -> joinRanges [rangeSpan a, rangeSpan b]

instance GetRange a => GetRange (Annoted a) where
  getRange x = case x of
    Annoted _ p _ _ -> p
  rangeSpan x = case x of
    Annoted a b c d -> joinRanges [rangeSpan a, rangeSpan b,
                                   rangeSpan c, rangeSpan d]

instance (GetRange s, GetRange a) => GetRange (SenAttr s a) where
  getRange = const nullRange
  rangeSpan x = case x of
    SenAttr a b c d e f g -> joinRanges [rangeSpan a, rangeSpan b,
