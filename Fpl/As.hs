...
instance GetRange FplExt where
  ...
  rangeSpan x = case x of
    FplSortItems a b -> joinRanges [rangeSpan a, rangeSpan b]
    FplOpItems a b -> joinRanges [rangeSpan a, rangeSpan b]

instance GetRange FplSortItem where
  getRange = const nullRange
  rangeSpan x = case x of
    FreeType a -> joinRanges [rangeSpan a]
    CaslSortItem a -> joinRanges [rangeSpan a]

instance GetRange FplOpItem where
  getRange = const nullRange
  rangeSpan x = case x of
    FunOp a -> joinRanges [rangeSpan a]
    CaslOpItem a -> joinRanges [rangeSpan a]

instance GetRange FunDef where
  getRange x = case x of
    FunDef _ _ _ p -> p
  rangeSpan x = case x of
    FunDef a b c d -> joinRanges [rangeSpan a, rangeSpan b,
...
instance GetRange TermExt where
  getRange x = case x of
    ...
    Case _ _ p -> p
    Let _ _ p -> p
    IfThenElse _ _ _ p -> p
    EqTerm _ _ p -> p
    BoolTerm _ -> nullRange
  rangeSpan x = case x of
    FixDef a -> joinRanges [rangeSpan a]
    Case a b c -> joinRanges [rangeSpan a, rangeSpan b, rangeSpan c]
    Let a b c -> joinRanges [rangeSpan a, rangeSpan b, rangeSpan c]
    IfThenElse a b c d -> joinRanges [rangeSpan a, rangeSpan b,
                                      rangeSpan c, rangeSpan d]
    EqTerm a b c -> joinRanges [rangeSpan a, rangeSpan b, rangeSpan c]
    BoolTerm a -> joinRanges [rangeSpan a]
